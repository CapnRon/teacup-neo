"""Add the new J37 (MSC1 breakout) footprint to the real board -- schematic-
only until now (build_headers.py/build_connector.py). Placed in the clear
gap just below J35 (the header it was carved out of), nothing else on the
board touched or moved. Per explicit user direction, 2026-07-14.
...
Run with the real KiCad 10.0.4 install (see build_pcb.py for why):
    LD_LIBRARY_PATH=/opt/kicad10/AppDir/shared/lib:/opt/kicad10/AppDir/usr/lib \
        /opt/kicad10/AppDir/bin/python3.11 add_j37_real_board.py
"""
import re
import pcbnew
...
if any(fp.GetReference() == "J37" for fp in board.GetFootprints()):
    raise SystemExit("J37 already on this board -- not adding a duplicate")

fp = pcbnew.FootprintLoad(LIBS["Connector_PinHeader_2.54mm"], "PinHeader_1x07_P2.54mm_Vertical")
fp.SetReference("J37")
fp.SetValue("HDR_J37")
fp.SetOrientationDegrees(90)
fp.SetPosition(pcbnew.VECTOR2I(mm(245.0), mm(50.0)))
board.Add(fp)

pin_to_net = {
    "1": "GND", "2": "MSC1_CLK", "3": "MSC1_CMD",
    "4": "MSC1_D0", "5": "MSC1_D1", "6": "MSC1_D2", "7": "MSC1_D3_CD",
}
for pad in fp.Pads():
...
# "MSC1" silkscreen label, matching every other header's style.
